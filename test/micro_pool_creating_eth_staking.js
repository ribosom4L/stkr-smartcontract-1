...
contract("MicroPool Creating and Staking (ETH Staking)", function(accounts) {
  let ankr;
  let staking;
  let micropool;
  let owner;
  ...
  it("allows users to create micro pool", async () => {
    ...
    await micropool.initializePoolWithETH(poolName, {
      value: helpers.amount(4)
    });
  ...
  it("should revert insufficient stake amounts", async () => {
    await expectRevert(micropool.initializePoolWithETH(
      helpers.makeHex("test"),
      { from: accounts[1], value: helpers.amount(3) }
