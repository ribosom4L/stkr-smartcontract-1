...
contract('MicroPool Creating and Staking', function (accounts) {
    ...
    it('should send exceed amount when pool balance is more than 32 ether', async () => {
        ...
        assert.equal((value - firstStaking), Number(await micropool.userStakeAmount(1, accounts[2])))
    })
    ...
    it('owner should be able to send push request', async () => {
        ...
        await expectRevert(micropool.pushToBeacon(1, depositData[0], depositData[1], depositData[2], depositData[3], {from: accounts[1]}), "Ownable: caller is not the owner")
    ...
    it('push requests should be reverted after success push', async () => {
        await expectRevert(micropool.pushToBeacon(1, depositData[0], depositData[1], depositData[2], depositData[3]), "Pool status not allow to push")
    })
